...
const CARD_GAP_MM = 2;
const GRID_START_X_MM = (PAGE_WIDTH_MM - (CARD_WIDTH_MM * 3 + CARD_GAP_MM * 2)) / 2;
const GRID_START_Y_MM = (PAGE_HEIGHT_MM - (CARD_HEIGHT_MM * 3 + CARD_GAP_MM * 2)) / 2;
      ...
      <div class="product-row${product.selected ? " is-selected" : ""}" data-product-id="${product.id}">
        ...
        <div class="product-main">
          ...
          <div class="ticket-row-controls" ${product.selected ? "" : "hidden"}>
            ...
            <label>シリアル<input class="ticket-serial-input" data-action="serial" type="text" maxlength="24" value="${escapeHtml(product.serial)}"></label>
            <button class="serial-refresh" data-action="refresh-serial" type="button" title="シリアル再生成">↻</button>
          </div>
        </div>
      </div>
    `;
...
function setProductSelected(product, selected) {
  ...
  if (selected) {
    if (!product.serial) product.serial = randomTicketSerial();
...
function buildTicketQueue() {
  ...
  getSelectedProducts().forEach(product => {
    ...
    for (let index = 0; index < quantity; index += 1) queue.push(product);
  });
  return queue;
}
...
function updateSelectionSummary() {
  ...
  elements.exportPdfButton.disabled = selectedProducts.length === 0 || state.exporting;
  ...
  elements.exportDetail.textContent = selectedProducts.length
    ? `A4 ${pageTotal}ページ／同一商品は同じシリアル`
    : "1商品につき初期9枚・同じシリアルで複製";
  renderA4Preview();
}

function miniTicketHtml(product) {
  const imageSource = normalizeImageUrl(product.imageUrl);
  return `
    <div class="mini-ticket">
      <img class="mini-logo" src="${escapeHtml(state.logoDataUrl)}" alt="">
      <div class="mini-title">${escapeHtml(state.settings.title)}</div>
      <div class="mini-subtitle">${escapeHtml(state.settings.subtitle)}</div>
      <div class="mini-image-stage">
        ${imageSource
          ? `<img src="${escapeHtml(imageSource)}" alt="" referrerpolicy="no-referrer" onerror="this.replaceWith(Object.assign(document.createElement('span'),{className:'mini-no-image',textContent:'NO IMAGE'}))">`
          : '<span class="mini-no-image">NO IMAGE</span>'}
      </div>
      <div class="mini-prize-name">${escapeHtml(product.name)}${product.type ? `<br>${escapeHtml(product.type)}` : ""}</div>
      <div class="mini-serial-label">SERIAL No.</div>
      <div class="mini-serial">${escapeHtml(normalizeSerial(product.serial))}</div>
      <div class="mini-notes">${escapeHtml(state.settings.notes)}</div>
    </div>
  `;
}
...
function renderA4Preview() {
  ...
  for (let index = 0; index < 9; index += 1) {
    ...
    card.style.left = `${(GRID_START_X_MM + column * (CARD_WIDTH_MM + CARD_GAP_MM)) / PAGE_WIDTH_MM * 100}%`;
    card.style.top = `${(GRID_START_Y_MM + row * (CARD_HEIGHT_MM + CARD_GAP_MM)) / PAGE_HEIGHT_MM * 100}%`;
    card.style.width = `${CARD_WIDTH_MM / PAGE_WIDTH_MM * 100}%`;
    card.style.height = `${CARD_HEIGHT_MM / PAGE_HEIGHT_MM * 100}%`;
...
function drawPrizeBanner(context, name, type) {
  const x = 88;
  const y = 617;
  const width = 568;
  const height = 74;
  ...
  if (type) {
    fitCanvasFont(context, name, 470, 25, 18, '"Noto Sans JP", "Yu Gothic", sans-serif');
    context.fillText(name, x + width / 2, y + 27);
    fitCanvasFont(context, type, 470, 17, 13, '"Noto Sans JP", "Yu Gothic", sans-serif', 800);
    context.fillText(type, x + width / 2, y + 53);
  } else {
    fitCanvasFont(context, name, 480, 29, 18, '"Noto Sans JP", "Yu Gothic", sans-serif');
    context.fillText(name, x + width / 2, y + height / 2 + 1);
  }
}

async function renderTicketDataUrl(product, imageDataUrl, logoDataUrl, settings) {
  ...
  context.strokeRect(8, 8, TICKET_WIDTH_PX - 16, TICKET_HEIGHT_PX - 16);
  context.strokeStyle = "#111111";
  context.lineWidth = 3;
  pathRoundedRect(context, 23, 23, TICKET_WIDTH_PX - 46, TICKET_HEIGHT_PX - 46, 8);
  context.stroke();
  ...
  context.fillRect(0, 0, TICKET_WIDTH_PX, 18);
  context.fillRect(0, TICKET_HEIGHT_PX - 18, TICKET_WIDTH_PX, 18);

  drawContainedImage(context, logoImage, 157, 48, 430, 116, 2);

  context.fillStyle = "#111111";
  context.textAlign = "center";
  context.textBaseline = "alphabetic";
  fitCanvasFont(context, settings.title, 580, 42, 28, '"Noto Sans JP", "Yu Gothic", sans-serif');
  context.fillText(settings.title, TICKET_WIDTH_PX / 2, 207);
  context.font = '900 24px Oswald, "Arial Narrow", sans-serif';
  context.fillText(settings.subtitle, TICKET_WIDTH_PX / 2, 237);

  context.save();
  context.shadowColor = "rgba(0,0,0,.18)";
  context.shadowBlur = 14;
  context.shadowOffsetY = 8;
  drawContainedImage(context, productImage, 112, 276, 520, 318, 8);
  context.restore();

  drawPrizeBanner(context, product.name, product.type);

  context.fillStyle = "#111111";
  context.font = '900 34px Oswald, "Arial Narrow", sans-serif';
  context.textAlign = "center";
  context.fillText("SERIAL No.", TICKET_WIDTH_PX / 2, 738);

  pathRoundedRect(context, 157, 756, 430, 62, 7);
  ...
  fitCanvasFont(context, normalizeSerial(product.serial), 380, 31, 23, 'Oswald, "Arial Narrow", monospace');
  context.textBaseline = "middle";
  context.fillText(normalizeSerial(product.serial), TICKET_WIDTH_PX / 2, 787);

  context.setLineDash([5, 6]);
  context.strokeStyle = "#caa33c";
  context.lineWidth = 3;
  context.beginPath();
  context.moveTo(66, 846);
  context.lineTo(TICKET_WIDTH_PX - 66, 846);
  context.stroke();
  context.setLineDash([]);
  ...
  context.font = '700 15px "Noto Sans JP", "Yu Gothic", sans-serif';
  const noteLines = wrapCanvasText(context, settings.notes, TICKET_WIDTH_PX - 152).slice(0, 8);
  noteLines.forEach((line, index) => context.fillText(line, 76, 861 + index * 20));

  return canvas.toDataURL("image/png");
}

function drawCropMarks(pdf, x, y) {
...
async function exportBatchPdf() {
  ...
  try {
    ...
    const ticketDataById = new Map();
    for (let index = 0; index < selectedProducts.length; index += 1) {
      const product = selectedProducts[index];
      const ticketData = await renderTicketDataUrl(product, fetchedImages[index], state.logoDataUrl, state.settings);
      ticketDataById.set(product.id, ticketData);
      setExportProgress(40 + (index + 1) / selectedProducts.length * 35, `引換券を生成中 ${index + 1}/${selectedProducts.length}`);
    }
    ...
    for (let index = 0; index < queue.length; index += 1) {
      ...
      const x = GRID_START_X_MM + column * (CARD_WIDTH_MM + CARD_GAP_MM);
      const y = GRID_START_Y_MM + row * (CARD_HEIGHT_MM + CARD_GAP_MM);
      const product = queue[index];
      const alias = `ticket-${product.id}-${String(product.serial).replace(/\D/g, "")}`;
      pdf.addImage(ticketDataById.get(product.id), "PNG", x, y, CARD_WIDTH_MM, CARD_HEIGHT_MM, alias, "FAST");
      drawCropMarks(pdf, x, y);
    ...
    elements.exportDetail.textContent = `A4 ${Math.ceil(ticketTotal / 9)}ページ／印刷は倍率100%`;
  } catch (error) {
    console.error(error);
    ...
    elements.exportPdfButton.disabled = !state.selectedOrder.length;
...
function handleProductListChange(event) {
  ...
  if (action === "select") {
    ...
    product.serial = normalizeSerial(event.target.value);
    event.target.value = product.serial;
    renderA4Preview();
  }
}
...
function handleProductListClick(event) {
  ...
  const product = state.products.find(item => item.id === Number(row?.dataset.productId));
  if (!product) return;
  product.serial = randomTicketSerial();
  const input = row.querySelector('[data-action="serial"]');
  if (input) input.value = product.serial;
  renderA4Preview();
}
